...
#include "DebaratiAODV.h"
using namespace inet;

Define_Module(DebaratiAODV);
const int KIND_DELAYEDSEND = 100;

void DebaratiAODV::initialize(int stage)
{
...
void DebaratiAODV::handleMessageWhenUp(cMessage *msg)
{
...
void DebaratiAODV::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
...
void DebaratiAODV::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result DebaratiAODV::ensureRouteForDatagram(Packet *datagram)
{
...
DebaratiAODV::DebaratiAODV()
{
}

bool DebaratiAODV::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void DebaratiAODV::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address DebaratiAODV::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void DebaratiAODV::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void DebaratiAODV::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void DebaratiAODV::sendRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rreq> DebaratiAODV::createRREQ(const L3Address& destAddr)
{
...
const Ptr<Rrep> DebaratiAODV::createRREP(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
...
const Ptr<Rrep> DebaratiAODV::createGratuitousRREP(const Ptr<Rreq>& rreq, IRoute *originatorRoute)
{
...
void DebaratiAODV::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void DebaratiAODV::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void DebaratiAODV::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void DebaratiAODV::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void DebaratiAODV::socketClosed(UdpSocket *socket)
{
...
void DebaratiAODV::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
...
IRoute *DebaratiAODV::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void DebaratiAODV::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void DebaratiAODV::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> DebaratiAODV::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void DebaratiAODV::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void DebaratiAODV::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void DebaratiAODV::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void DebaratiAODV::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void DebaratiAODV::clearState()
{
...
void DebaratiAODV::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
...
void DebaratiAODV::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void DebaratiAODV::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void DebaratiAODV::forwardRREQ(const Ptr<Rreq>& rreq, unsigned int timeToLive)
{
...
void DebaratiAODV::completeRouteDiscovery(const L3Address& target)
{
...
void DebaratiAODV::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rrep> DebaratiAODV::createHelloMessage()
{
...
void DebaratiAODV::sendHelloMessagesIfNeeded()
{
...
void DebaratiAODV::handleHelloMessage(const Ptr<Rrep>& helloMessage)
{
...
void DebaratiAODV:: InBangDinhTuyen()
{
    EV_INFO <<"--- --------------ROUTING TABBLE at " <<simTime() <<endl;
    ...
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
            ...
            if (route->getSource() == this) { // Chinhs la node hien hanh
                 ...
                 EV_INFO <<"| -- "<<  sequenceNum;
                 EV_INFO <<"| -- "<<  routeData ->getDestSeqNum() <<endl;
            }
    }
}
void DebaratiAODV::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        IRoute *route = routingTable->getRoute(i);
        if (route->getSource() == this) {
...
void DebaratiAODV::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result DebaratiAODV::datagramForwardHook(Packet *datagram)
{
...
void DebaratiAODV::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void DebaratiAODV::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool DebaratiAODV::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> DebaratiAODV::createRREPACK()
{
...
void DebaratiAODV::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void DebaratiAODV::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void DebaratiAODV::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void DebaratiAODV::handleBlackListTimer()
{
