...
#include "DebaratiAODV_BH.h"
using namespace inet;

Define_Module(DebaratiAODV_BH);
const int KIND_DELAYEDSEND = 100;


/* NTU_ATTACKS: handle message from controller. */
void DebaratiAODV_BH::handleMessageFromAttackController(cMessage *msg){
...
void DebaratiAODV_BH::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        DSNvsSSN =2000000;                      //thomc hack Debarati
        dichLaNodeLangGieng = false;
...
void DebaratiAODV_BH::handleMessageWhenUp(cMessage *msg)
{
...
void DebaratiAODV_BH::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
...
void DebaratiAODV_BH::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result DebaratiAODV_BH::ensureRouteForDatagram(Packet *datagram)
{
...
DebaratiAODV_BH::DebaratiAODV_BH()
{
}

bool DebaratiAODV_BH::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void DebaratiAODV_BH::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address DebaratiAODV_BH::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void DebaratiAODV_BH::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void DebaratiAODV_BH::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void DebaratiAODV_BH::sendRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rreq> DebaratiAODV_BH::createRREQ(const L3Address& destAddr)
{
...
const Ptr<Rrep> DebaratiAODV_BH::createRREP(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
    ...
    if (rreq->getDestAddr() == getSelfIPAddress()) {    // node is itself the requested destination
        ...
        rrep->setRREP_Generator_Address(getSelfIPAddress());  // modify, add by thomc
    }
...
const Ptr<Rrep> DebaratiAODV_BH::createGratuitousRREP(const Ptr<Rreq>& rreq, IRoute *originatorRoute)
{
...
void DebaratiAODV_BH::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void DebaratiAODV_BH::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void DebaratiAODV_BH::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void DebaratiAODV_BH::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void DebaratiAODV_BH::socketClosed(UdpSocket *socket)
{
...
void DebaratiAODV_BH::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
...
IRoute *DebaratiAODV_BH::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void DebaratiAODV_BH::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void DebaratiAODV_BH::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> DebaratiAODV_BH::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void DebaratiAODV_BH::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void DebaratiAODV_BH::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void DebaratiAODV_BH::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void DebaratiAODV_BH::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void DebaratiAODV_BH::clearState()
{
...
void DebaratiAODV_BH::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
    // Chú ý trường hợp là Node láng giêng gửi Rrep  (Có hopcout =0 á )
    //thomc hack: Pre-rrep processing
    // Xử lý nếu là gói đã về đến node gửi
    simtime_t lifeTimeDebarati = rrep->getLifeTime();
    if ((getSelfIPAddress() == rrep->getOriginatorAddr()))  {  // nut nguồn
        EV_INFO << "Xử lý RREP ở nguồn" <<endl;
        if (rrep->getHopCount()==0)  // Node láng gieieengf bình thưởng
        {
            EV_INFO << "RREP from my trust neibouger:" << rrep->getDestAddr() << endl;
            dichLaNodeLangGieng =true;
        } // Chý ý, nếu láng giềng là độc hại, thì Hopcout bằng 1 (do ta cài đặt vậy)
        if (hasOngoingRouteDiscovery(rrep->getDestAddr())) {   // và chưa timeout
            EV_INFO << "The Route Reply has arrived for our Route Request to node " << rrep->getDestAddr() << endl;
            srcAdr_RR_Table[numRrep] =sourceAddr;
            RR_Table[numRrep]=rrep;
            EV_INFO <<"Src:" << sourceAddr << " Dest: " <<RR_Table[numRrep]->getDestAddr() << " HC: " <<RR_Table[numRrep]->getHopCount() << "DSN:   " <<RR_Table[numRrep]->getDestSeqNum() <<endl;
            numRrep++;
        }
    } // không phải là nút nguồn
    else
    {
        EV_INFO << "Xử lý RREP ở nút trung gian" <<endl;
        // Xử lý bình thường, sử dụng bản gốc
//     if (rrep->getDestSeqNum()<DSNvsSSN+sequenceNum) // không khác quá nhiều
//     {   // Xử lý bình thường, sử dụng bản gốc
    ...
    simtime_t lifeTime = rrep->getLifeTime();  // thomc hack
 ...
 //} // ENd Xử lý bình thường, sử dụng bản gốc -- DSN không khác quá nhiều SSN
    } // ENd Xử lý bình thường, sử dụng bản gốc
}

void DebaratiAODV_BH::handleWaitForRREP(WaitForRrep *rrepTimer)
{

    EV_INFO <<"Het time doi RREP: numREP=" << numRrep <<endl;

    if (numRrep == 0) {  // Timout do khong doi duong RREP
    ...
    } else if (numRrep>0) //, has mininum one RREP packet
        {
            EV_INFO << "One round timeout with num Rrep= " <<numRrep<<endl;
            // In ra để xem
            for (int i=0;i<numRrep; i++){
                EV_INFO << "Add to RR-Table: OriginatorAddr" <<RR_Table[i]->getOriginatorAddr() <<endl;
                EV_INFO << "Dest: " <<RR_Table[i]->getDestAddr() <<" Nexthop:"<< srcAdr_RR_Table[i] << " HC:" <<RR_Table[i]->getHopCount() << " DSN:" <<RR_Table[i]->getDestSeqNum() <<endl;
             } // for

           // Sắp xếp giảm dẫn theo DSN
            for (int i=0;i<numRrep-1; i++)
                for (int j=i+1; j<numRrep; j++){
                    if (RR_Table[i]->getDestSeqNum()< RR_Table[j]->getDestSeqNum()){
                                Ptr<Rrep> &rrepTemp =RR_Table[i];
                                    RR_Table[i] = RR_Table[j];
                                    RR_Table[j] =rrepTemp;
                                }
                        }
//            //  Xử lý các bỏ qua các first entry, có DSN>>>>SSN
            int indexStart=0;
            while (indexStart<numRrep)
                {
                    if ((RR_Table[indexStart]->getDestSeqNum()) >(DSNvsSSN+sequenceNum)) indexStart ++;  // Bỏ qua
                    else break;
                }
            EV_INFO <<"Số lượng bỏ qua: "<< indexStart;
            if (indexStart==numRrep)
            {
                numRrep=0;
                EV_INFO << "We didn't get any Route Reply within RREP timeout" << endl;
                L3Address destAddr = rrepTimer->getDestAddr();
...
//
//            indexStart=0;
            Ptr<Rrep> &rrepMin =RR_Table[indexStart];
            L3Address srcAddress = srcAdr_RR_Table[indexStart];  // dia chi node vua gui RREP
            EV_INFO << "CHON: Dest: " <<rrepMin->getDestAddr() << "HC: " <<rrepMin->getHopCount() << "DSN:   " <<rrepMin->getDestSeqNum() <<endl;
            EV_INFO <<"Next hop:" <<srcAdr_RR_Table[indexStart];

            IRoute *destRouteChosen = routingTable->findBestMatchingRoute(rrepMin->getDestAddr());

            // Nếu bảng định tuyến đang rỗng thì tạo mới
            if (destRouteChosen != NULL)
            {
                EV_INFO <<"No Entry for " <<  rrepMin->getDestAddr();
                destRouteChosen= createRoute(rrepMin->getDestAddr(), srcAddress, 1, false, rrepMin->getDestSeqNum(), true, simTime() + activeRouteTimeout);
            }
           // Ngược lại mới cập nhật
             unsigned int newHopCountChosen =rrepMin->getHopCount()+1;
             updateRoutingTable(destRouteChosen, srcAddress , newHopCountChosen, true, rrepMin->getDestSeqNum(), true, simTime() + rrepMin->getLifeTime());
             completeRouteDiscovery(rrepMin->getDestAddr());
            }
        }
    numRrep=0;
...
void DebaratiAODV_BH::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void DebaratiAODV_BH::forwardRREQ(const Ptr<Rreq>& rreq, unsigned int timeToLive)
{
...
void DebaratiAODV_BH::completeRouteDiscovery(const L3Address& target)
{
...
void DebaratiAODV_BH::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rrep> DebaratiAODV_BH::createHelloMessage()
{
...
void DebaratiAODV_BH::sendHelloMessagesIfNeeded()
{
...
void DebaratiAODV_BH::handleHelloMessage(const Ptr<Rrep>& helloMessage)
{
...
void DebaratiAODV_BH::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        IRoute *route = routingTable->getRoute(i);
        if (route->getSource() == this) {
...
void DebaratiAODV_BH::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result DebaratiAODV_BH::datagramForwardHook(Packet *datagram)
{
...
void DebaratiAODV_BH::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void DebaratiAODV_BH::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool DebaratiAODV_BH::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> DebaratiAODV_BH::createRREPACK()
{
...
void DebaratiAODV_BH::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void DebaratiAODV_BH::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void DebaratiAODV_BH::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void DebaratiAODV_BH::handleBlackListTimer()
{
