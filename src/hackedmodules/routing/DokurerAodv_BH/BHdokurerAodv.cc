...
#include "BHdokurerAodv.h"
using namespace inet;


Define_Module(BHdokurerAodv);

const int KIND_DELAYEDSEND = 100;

/* NTU_ATTACKS: handle message from controller. */
void BHdokurerAodv::handleMessageFromAttackController(cMessage *msg){
  ...
  //      delete(msg);
    ...
    //    delete(msg);
     // END NTU_GRAYKHOLE -
        ...
        LOG << "ERROR: Message unknown in BHdokurerAodv::handleMessageFromAttackController. Msg: " << msg->getFullName() << "\n";
    }
}

//----------------end attack code----------------------
void BHdokurerAodv::initialize(int stage)
{
...
void BHdokurerAodv::handleMessageWhenUp(cMessage *msg)
{
...
void BHdokurerAodv::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
...
void BHdokurerAodv::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result BHdokurerAodv::ensureRouteForDatagram(Packet *datagram)
{
...
BHdokurerAodv::BHdokurerAodv()
{

}

bool BHdokurerAodv::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void BHdokurerAodv::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address BHdokurerAodv::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void BHdokurerAodv::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void BHdokurerAodv::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void BHdokurerAodv::sendRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rreq> BHdokurerAodv::createRREQ(const L3Address& destAddr)
{
...
const Ptr<Rrep> BHdokurerAodv::createRREP(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
...
const Ptr<Rrep> BHdokurerAodv::createGratuitousRREP(const Ptr<Rreq>& rreq, IRoute *originatorRoute)
{
...
void BHdokurerAodv::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
    int NGUONG =1000;
    ...
    if (getSelfIPAddress() != rrep->getOriginatorAddr()) {
        ...
        if (hasOngoingRouteDiscovery(rrep->getDestAddr())) {
                    ...
                    if (numRrep>=2)
                    {
                        ...
                        EV_INFO << "RR-Table: OriginatorAddr (Sender x)" <<RR_Table[0]->getOriginatorAddr() <<endl;
                        for (int i=0;i<numRrep; i++){
                            EV_INFO << "Dest: " <<RR_Table[i]->getDestAddr() << "HC: " <<RR_Table[i]->getHopCount() << "DSN:   " <<RR_Table[i]->getDestSeqNum() <<endl;

                                      }

                                      Ptr<Rrep> &rrepMin =RR_Table[0];
                                      for (int i=1;i<numRrep; i++){
                                          if (RR_Table[i]->getDestSeqNum()!=4294967295)
                                              if (RR_Table[i]->getHopCount()<= rrepMin->getHopCount())
                                                  if (RR_Table[i]->getDestSeqNum()>= rrepMin->getDestSeqNum())
                                              {
                                                                       rrepMin =RR_Table[i];
                                              }
                                      }
                                      ...
                                      EV_INFO << "CuongTho" << rrep->getDestAddr() << endl;
                                      updateRoutingTable(destRouteChosen, sourceAddr, newHopCountChosen, true, rrepMin->getDestSeqNum(), true, simTime() + lifeTime);
...
void BHdokurerAodv::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void BHdokurerAodv::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void BHdokurerAodv::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void BHdokurerAodv::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void BHdokurerAodv::socketClosed(UdpSocket *socket)
{
...
void BHdokurerAodv::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
...
IRoute *BHdokurerAodv::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void BHdokurerAodv::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void BHdokurerAodv::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> BHdokurerAodv::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void BHdokurerAodv::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void BHdokurerAodv::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void BHdokurerAodv::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void BHdokurerAodv::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void BHdokurerAodv::clearState()
{
...
void BHdokurerAodv::handleWaitForRREP(WaitForRrep *rrepTimer)
{
//    if (numRrep==0) {
...
//               auto waitRREPIter = waitForRREPTimers.find(rrepTimer->getDestAddr());
...
void BHdokurerAodv::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void BHdokurerAodv::forwardRREQ(const Ptr<Rreq>& rreq, unsigned int timeToLive)
{
...
void BHdokurerAodv::completeRouteDiscovery(const L3Address& target)
{
...
void BHdokurerAodv::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rrep> BHdokurerAodv::createHelloMessage()
{
...
void BHdokurerAodv::sendHelloMessagesIfNeeded()
{
...
void BHdokurerAodv::handleHelloMessage(const Ptr<Rrep>& helloMessage)
{
...
void BHdokurerAodv::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        IRoute *route = routingTable->getRoute(i);
        if (route->getSource() == this) {
...
void BHdokurerAodv::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result BHdokurerAodv::datagramForwardHook(Packet *datagram)
{
...
void BHdokurerAodv::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void BHdokurerAodv::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool BHdokurerAodv::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> BHdokurerAodv::createRREPACK()
{
...
void BHdokurerAodv::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void BHdokurerAodv::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void BHdokurerAodv::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void BHdokurerAodv::handleBlackListTimer()
{
