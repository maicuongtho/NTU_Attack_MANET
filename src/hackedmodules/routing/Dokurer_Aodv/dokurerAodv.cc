...
#include "dokurerAodv.h"
using namespace inet;

Define_Module(dokurerAodv);

const int KIND_DELAYEDSEND = 100;

void dokurerAodv::initialize(int stage)
{
...
void dokurerAodv::handleMessageWhenUp(cMessage *msg)
{
...
void dokurerAodv::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
...
void dokurerAodv::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result dokurerAodv::ensureRouteForDatagram(Packet *datagram)
{
...
dokurerAodv::dokurerAodv()
{
}

bool dokurerAodv::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void dokurerAodv::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address dokurerAodv::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void dokurerAodv::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void dokurerAodv::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void dokurerAodv::sendRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rreq> dokurerAodv::createRREQ(const L3Address& destAddr)
{
...
const Ptr<Rrep> dokurerAodv::createRREP(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
...
const Ptr<Rrep> dokurerAodv::createGratuitousRREP(const Ptr<Rreq>& rreq, IRoute *originatorRoute)
{
...
void dokurerAodv::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
    int NGUONG =1000;
    ...
    if (destRoute && destRoute->getSource() == this) {    // already exists
        ...
        //      known value is valid, or
            ...
            // Dong y cap nhat lo trinh moi, neu lon hon hien taij
            // nhung khong duoc qua lon,vi mallicuos
            if (destSeqNum <NGUONG)  // thomc
                updateRoutingTable(destRoute, sourceAddr, newHopCount, true, destSeqNum, true, simTime() + lifeTime);
        }
        else {
...
//              EV_INFO << "One round timeout with num Rrep= " <<numRrep<<endl;
//              for (int i=0;i<numRrep; i++){
//                  EV_INFO << "Add to RR-Table: OriginatorAddr (Sender x)   " <<RR_Table[i]->getOriginatorAddr() <<endl;
//                  EV_INFO << "Dest: " <<RR_Table[i]->getDestAddr() << "HC: " <<RR_Table[i]->getHopCount() << "DSN:   " <<RR_Table[i]->getDestSeqNum() <<endl;
//
//              }
//
...
void dokurerAodv::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void dokurerAodv::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void dokurerAodv::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void dokurerAodv::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void dokurerAodv::socketClosed(UdpSocket *socket)
{
...
void dokurerAodv::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
...
IRoute *dokurerAodv::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void dokurerAodv::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void dokurerAodv::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> dokurerAodv::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void dokurerAodv::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void dokurerAodv::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void dokurerAodv::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void dokurerAodv::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void dokurerAodv::clearState()
{
...
void dokurerAodv::handleWaitForRREP(WaitForRrep *rrepTimer)
{
//    if (numRrep==0) {
...
void dokurerAodv::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void dokurerAodv::forwardRREQ(const Ptr<Rreq>& rreq, unsigned int timeToLive)
{
...
void dokurerAodv::completeRouteDiscovery(const L3Address& target)
{
...
void dokurerAodv::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rrep> dokurerAodv::createHelloMessage()
{
...
void dokurerAodv::sendHelloMessagesIfNeeded()
{
...
void dokurerAodv::handleHelloMessage(const Ptr<Rrep>& helloMessage)
{
...
void dokurerAodv::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        IRoute *route = routingTable->getRoute(i);
        if (route->getSource() == this) {
...
void dokurerAodv::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result dokurerAodv::datagramForwardHook(Packet *datagram)
{
...
void dokurerAodv::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void dokurerAodv::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool dokurerAodv::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> dokurerAodv::createRREPACK()
{
...
void dokurerAodv::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void dokurerAodv::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void dokurerAodv::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void dokurerAodv::handleBlackListTimer()
{
