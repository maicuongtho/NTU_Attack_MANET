...
Define_Module(hhsAodv);

const int KIND_DELAYEDSEND = 100;


void hhsAodv::initialize(int stage)
{
    //thomc
    firstRrep=true;
...
void hhsAodv::handleMessageWhenUp(cMessage *msg)
{
...
void hhsAodv::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
...
void hhsAodv::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result hhsAodv::ensureRouteForDatagram(Packet *datagram)
{
...
bool hhsAodv::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void hhsAodv::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address hhsAodv::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void hhsAodv::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void hhsAodv::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
    //thomc
    firstRrep=true;
...
void hhsAodv::sendRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rreq> hhsAodv::createRREQ(const L3Address& destAddr)
{
...
const Ptr<Rrep> hhsAodv::createRREP(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
...
const Ptr<Rrep> hhsAodv::createGratuitousRREP(const Ptr<Rreq>& rreq, IRoute *originatorRoute)
{
...
void hhsAodv::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
    ...
    else { //if (getSelfIPAddress() == rrep->getOriginatorAddr())
...
//               }
        ...
        if (hasOngoingRouteDiscovery(rrep->getDestAddr())) {
            EV_INFO << "The Route Reply has arrived for our Route Request to node " << rrep->getDestAddr() << endl;
            lstRrep[numRrep] = rrep;
            numRrep++;
            if (numRrep>=4){ // Tuy thuoc so nut mang
                          Ptr<Rrep> &rrepMin =lstRrep[0];
                          for (int i=1;i<numRrep; i++){
                              if(rrepMin->getDestSeqNum()>lstRrep[i]->getDestSeqNum()){
                                  rrepMin =lstRrep[i];
                              }
                          }
                          ...
                          numRrep=0;
                    }  // Het time kham pha tuyen
        }

//        WaitForRrep *taora =waitForRREPTimers.find(rrep->getDestAddr())->second;
//        simtime_t_cref timeCreated_init = taora->getCreationTime();
//        auto tam= timeCreated_init;
//        EV_INFO << "Thoi diem tao thong diep WaitForRrep:" << tam << "thu" << endl;
//        WaitForRrep *ketthuc =waitForRREPTimers.end()->second;
//        simtime_t_cref timeCreated_end  = ketthuc->getCreationTime();
    //    auto tam2 = timeCreated_end;
    //    EV_DETAIL << "Time create message end wait RReP" << tam2 << endl;
//        if (waitForRREPTimers.find(rrep->getDestAddr()) == waitForRREPTimers.end()) {

            //          unsigned int thoiLuonghet = pathDiscoveryTime;


    }
}

void hhsAodv::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void hhsAodv::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void hhsAodv::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void hhsAodv::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void hhsAodv::socketClosed(UdpSocket *socket)
{
...
void hhsAodv::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
...
IRoute *hhsAodv::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void hhsAodv::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void hhsAodv::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> hhsAodv::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void hhsAodv::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void hhsAodv::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void hhsAodv::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void hhsAodv::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void hhsAodv::clearState()
{
...
void hhsAodv::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void hhsAodv::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void hhsAodv::forwardRREQ(const Ptr<Rreq>& rreq, unsigned int timeToLive)
{
...
void hhsAodv::completeRouteDiscovery(const L3Address& target)
{
...
void hhsAodv::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rrep> hhsAodv::createHelloMessage()
{
...
void hhsAodv::sendHelloMessagesIfNeeded()
{
...
void hhsAodv::handleHelloMessage(const Ptr<Rrep>& helloMessage)
{
...
void hhsAodv::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result hhsAodv::datagramForwardHook(Packet *datagram)
{
...
void hhsAodv::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void hhsAodv::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool hhsAodv::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> hhsAodv::createRREPACK()
{
...
void hhsAodv::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void hhsAodv::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void hhsAodv::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void hhsAodv::handleBlackListTimer()
{
