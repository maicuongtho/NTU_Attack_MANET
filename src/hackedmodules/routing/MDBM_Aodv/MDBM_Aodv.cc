...
Define_Module(MDBM_Aodv);
const int KIND_DELAYEDSEND = 100;

void MDBM_Aodv::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        ttlFakeRREQThreshold=par(ttlFakeRREQThreshold); //thomc
        ...
        // thomc addd
        fakeRREQTimer = new cMessage("fakeRREQTimer");
        if (useHelloMessages)
...
void MDBM_Aodv::handleMessageWhenUp(cMessage *msg)
{
...
void MDBM_Aodv::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
...
void MDBM_Aodv::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result MDBM_Aodv::ensureRouteForDatagram(Packet *datagram)
{
...
bool MDBM_Aodv::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void MDBM_Aodv::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address MDBM_Aodv::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void MDBM_Aodv::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void MDBM_Aodv::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void MDBM_Aodv::sendRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rreq> MDBM_Aodv::createRREQ(const L3Address& destAddr)
{
...
const Ptr<Rreq> MDBM_Aodv::createFakeRREQ(const L3Address& destAddr)
{
    auto rreqPacket = makeShared<Rreq>(); // TODO: "AODV-RREQ");
    rreqPacket->setPacketType(usingIpv6 ? RREQ_IPv6 : RREQ);
    rreqPacket->setChunkLength(usingIpv6 ? B(48) : B(24));
...
const Ptr<Rrep> MDBM_Aodv::createRREP(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
...
const Ptr<Rrep> MDBM_Aodv::createGratuitousRREP(const Ptr<Rreq>& rreq, IRoute *originatorRoute)
{
...
void MDBM_Aodv::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
...
void MDBM_Aodv::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void MDBM_Aodv::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void MDBM_Aodv::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void MDBM_Aodv::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void MDBM_Aodv::socketClosed(UdpSocket *socket)
{
...
void MDBM_Aodv::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
...
IRoute *MDBM_Aodv::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void MDBM_Aodv::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void MDBM_Aodv::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> MDBM_Aodv::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void MDBM_Aodv::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void MDBM_Aodv::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void MDBM_Aodv::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void MDBM_Aodv::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void MDBM_Aodv::clearState()
{
...
void MDBM_Aodv::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void MDBM_Aodv::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void MDBM_Aodv::forwardRREQ(const Ptr<Rreq>& rreq, unsigned int timeToLive)
{
...
void MDBM_Aodv::completeRouteDiscovery(const L3Address& target)
{
...
void MDBM_Aodv::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rrep> MDBM_Aodv::createHelloMessage()
{
...
void MDBM_Aodv::sendHelloMessagesIfNeeded()
{
...
void MDBM_Aodv::handleHelloMessage(const Ptr<Rrep>& helloMessage)
{
...
void MDBM_Aodv::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result MDBM_Aodv::datagramForwardHook(Packet *datagram)
{
...
void MDBM_Aodv::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void MDBM_Aodv::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool MDBM_Aodv::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> MDBM_Aodv::createRREPACK()
{
...
void MDBM_Aodv::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void MDBM_Aodv::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void MDBM_Aodv::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void MDBM_Aodv::handleBlackListTimer()
{
...
// thomc add
/*  The source node is embedded with a bait timer (Bait Time ), and
that timer value is set randomly to A seconds. Whenever the timer reaches to A seconds,
the source node creates a Fake RREQ packet and broadcast it into the network with a
randomly generated fake destination address
*/
void MDBM_Aodv::sendFakeRREQ()
{
    if (lastBroadcastTimeFakeRREQ == 0 || simTime() - lastBroadcastTimeFakeRREQ > fakeRREQInterval) {
        EV_INFO << "It is fake RREQ time, broadcasting Fake RREQ with TTL="<< ttlFakeRREQThreshold << endl;
        Ipv4Address addr;
        addr.set("1.2.3.4");
        L3Address destAddr;
        destAddr.set(addr);
        auto fakeRREQ= createFakeRREQ(destAddr);
        sendRREQ(fakeRREQ, destAddr, ttlFakeRREQThreshold);
    }
    scheduleAt(simTime() + fakeRREQInterval - *periodicJitter, fakeRREQTimer);
}
