...
#include "niraAodv.h"
Define_Module(niraAodv);
const int KIND_DELAYEDSEND = 100;

void niraAodv::initialize(int stage)
{
...
void niraAodv::handleMessageWhenUp(cMessage *msg)
{
...
void niraAodv::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
...
void niraAodv::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result niraAodv::ensureRouteForDatagram(Packet *datagram)
{
...
bool niraAodv::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void niraAodv::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address niraAodv::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void niraAodv::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void niraAodv::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void niraAodv::sendRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rreq> niraAodv::createRREQ(const L3Address& destAddr)
{
...
const Ptr<Rrep> niraAodv::createRREP(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
...
const Ptr<Rrep> niraAodv::createGratuitousRREP(const Ptr<Rreq>& rreq, IRoute *originatorRoute)
{
...
void niraAodv::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
    // thomc  - check Trust Score in Rrep packet, if ==1 then return
    // else  6.7

    if (rrep->getTrustScoreNira()==0) {
        EV_INFO << "AODV Route Reply arrived with source address: " << sourceAddr << " originator addr: " << rrep->getOriginatorAddr()
                   << " destination address: " << rrep->getDestAddr() << endl;
        EV_INFO <<"But my neibouge node send me a FakeRREP, so I discast process"<<endl;
        return;
    }
...
void niraAodv::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void niraAodv::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void niraAodv::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void niraAodv::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void niraAodv::socketClosed(UdpSocket *socket)
{
...
void niraAodv::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
    ...
    // check (i)   // RREQ packet had arrived destination
    if (rreq->getDestAddr() == getSelfIPAddress()) {
        ...
        // --- thomc, step 6; data packet successfully transmitted
        EV_INFO << "RREQ reached destination. Increase Trust Score " << TrustScoreNira +1<< endl;
        TrustScoreNira ++;
        rrep->setTrustScoreNira(TrustScoreNira);
        // --/thomc
    ...
    //TrustScoreNira=0; // strp 6: otherwise, it becomes zero
...
IRoute *niraAodv::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void niraAodv::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void niraAodv::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> niraAodv::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void niraAodv::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void niraAodv::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void niraAodv::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void niraAodv::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void niraAodv::clearState()
{
...
void niraAodv::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void niraAodv::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void niraAodv::forwardRREQ(const Ptr<Rreq>& rreq, unsigned int timeToLive)
{
...
void niraAodv::completeRouteDiscovery(const L3Address& target)
{
...
void niraAodv::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rrep> niraAodv::createHelloMessage()
{
...
void niraAodv::sendHelloMessagesIfNeeded()
{
...
void niraAodv::handleHelloMessage(const Ptr<Rrep>& helloMessage)
{
...
void niraAodv::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result niraAodv::datagramForwardHook(Packet *datagram)
{
...
void niraAodv::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void niraAodv::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool niraAodv::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> niraAodv::createRREPACK()
{
...
void niraAodv::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void niraAodv::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void niraAodv::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void niraAodv::handleBlackListTimer()
{
