...
#include "RrepAuthAodv.h"
Define_Module(RrepAuthAodv);
const int KIND_DELAYEDSEND = 100;
int RrepAuthAodv::modulo(long x, int n)
{
    return 0;
}
void RrepAuthAodv::initialize(int stage)
{
    ...
    if (stage == INITSTAGE_LOCAL) {
        ...
        //thomc; lấy các key từ tham số
        modulo_n = par("modulo_n");     // n=p*q; p,q là hai số nguyên tố
        publickey_e= par("publickey_e");   // Khóa công khai, dùng để xác thực thông tin
        privatekey_d=par("privatekey_d");   // Khóa bí mật, dùng để mã hóa địa chỉ IP
        //
...
void RrepAuthAodv::handleMessageWhenUp(cMessage *msg)
{
...
void RrepAuthAodv::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
...
void RrepAuthAodv::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result RrepAuthAodv::ensureRouteForDatagram(Packet *datagram)
{
...
RrepAuthAodv::RrepAuthAodv()
{
}

bool RrepAuthAodv::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void RrepAuthAodv::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address RrepAuthAodv::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void RrepAuthAodv::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void RrepAuthAodv::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void RrepAuthAodv::sendRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rreq> RrepAuthAodv::createRREQ(const L3Address& destAddr)
{
...
const Ptr<Rrep> RrepAuthAodv::createRREP(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
...
const Ptr<Rrep> RrepAuthAodv::createGratuitousRREP(const Ptr<Rreq>& rreq, IRoute *originatorRoute)
{
...
void RrepAuthAodv::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
    ...
    else   // else if 659, tuc la: if (getSelfIPAddress() == rrep->getOriginatorAddr())
    {       //RREP trở về đúng nút gửi RREQ
        if (hasOngoingRouteDiscovery(rrep->getDestAddr())) {
            // thomc, kiem tra chung thuc
            bool isRrepAuth=false;   // thomc
            //xư lý kiểm tra
                // lấy RREQ id


            isRrepAuth=true;   // for test
            if (isRrepAuth) {  // Nếu gói đến được xác thực
                EV_INFO << "The Route Reply has arrived for our Route Request to node " << rrep->getDestAddr() << endl;
                updateRoutingTable(destRoute, sourceAddr, newHopCount, true, destSeqNum, true, simTime() + lifeTime);

                //"Completing route discovery, originator " << getSelfIPAddress() << ", target " << target << endl;
...
void RrepAuthAodv::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void RrepAuthAodv::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void RrepAuthAodv::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void RrepAuthAodv::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void RrepAuthAodv::socketClosed(UdpSocket *socket)
{
...
void RrepAuthAodv::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
    ...
    if (rreq->getDestAddr() == getSelfIPAddress()) {
        ...
        // thomc: Mã hoá các ocet địa chỉ IP của mình với khóa bí mật
        int octet1 = getSelfIPAddress().toIpv4().getDByte(0);
        int octet2 = getSelfIPAddress().toIpv4().getDByte(1);
        int octet3 = getSelfIPAddress().toIpv4().getDByte(2);
        int octet4 = getSelfIPAddress().toIpv4().getDByte(3);
        int octet1Diget = modulo(pow(octet1,privatekey_d), modulo_n);
        int octet2Diget = modulo(pow(octet2,privatekey_d), modulo_n);
        int octet3Diget = modulo(pow(octet3,privatekey_d), modulo_n);
        int octet4Diget = modulo(pow(octet4,privatekey_d), modulo_n);
        rrep->setIPaddressDiget_Octet1(octet1Diget);
        rrep->setIPaddressDiget_Octet2(octet2Diget);
        rrep->setIPaddressDiget_Octet3(octet3Diget);
        rrep->setIPaddressDiget_Octet4(octet4Diget);
...
IRoute *RrepAuthAodv::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void RrepAuthAodv::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void RrepAuthAodv::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> RrepAuthAodv::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void RrepAuthAodv::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void RrepAuthAodv::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void RrepAuthAodv::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void RrepAuthAodv::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void RrepAuthAodv::clearState()
{
...
void RrepAuthAodv::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void RrepAuthAodv::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void RrepAuthAodv::forwardRREQ(const Ptr<Rreq>& rreq, unsigned int timeToLive)
{
...
void RrepAuthAodv::completeRouteDiscovery(const L3Address& target)
{
...
void RrepAuthAodv::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rrep> RrepAuthAodv::createHelloMessage()
{
...
void RrepAuthAodv::sendHelloMessagesIfNeeded()
{
...
void RrepAuthAodv::handleHelloMessage(const Ptr<Rrep>& helloMessage)
{
...
void RrepAuthAodv::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        IRoute *route = routingTable->getRoute(i);
        if (route->getSource() == this) {
...
void RrepAuthAodv::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result RrepAuthAodv::datagramForwardHook(Packet *datagram)
{
...
void RrepAuthAodv::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void RrepAuthAodv::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool RrepAuthAodv::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> RrepAuthAodv::createRREPACK()
{
...
void RrepAuthAodv::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void RrepAuthAodv::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void RrepAuthAodv::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void RrepAuthAodv::handleBlackListTimer()
{
