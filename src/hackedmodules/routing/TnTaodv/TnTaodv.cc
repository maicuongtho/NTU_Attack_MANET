...
Define_Module(TnTaodv);

const int KIND_DELAYEDSEND = 100;

void TnTaodv::initialize(int stage)
{
...
void TnTaodv::handleMessageWhenUp(cMessage *msg)
{
...
void TnTaodv::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
...
void TnTaodv::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result TnTaodv::ensureRouteForDatagram(Packet *datagram)
{
...
void TnTaodv::PrintOutmyRoutingTable(){
...
bool TnTaodv::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void TnTaodv::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address TnTaodv::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void TnTaodv::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void TnTaodv::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
    mynumRREQ++;
...
void TnTaodv::sendRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rreq> TnTaodv::createRREQ(const L3Address& destAddr)
{
...
const Ptr<Rrep> TnTaodv::createRREP(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
    ...
    //hack
   // EV_INFO<<"Node create RREP, at destination node"<<endl;
...
const Ptr<Rrep> TnTaodv::createGratuitousRREP(const Ptr<Rreq>& rreq, IRoute *originatorRoute)
{
...
void TnTaodv::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
    EV_INFO << "Last Broadcast time :" <<lastBroadcastTime <<endl;
    EV_INFO << "Nhan RREP (s):" <<simTime()<<endl;
    EV_INFO << "Tresponse (RREP vs RREQ) = "<< simTime() - lastBroadcastTime <<endl;
    ...
    //thomc hack
    EV_INFO<<"I heard ---------------------------------------"<<endl;
    for (auto it = numRREQ_forwarded.begin(); it != numRREQ_forwarded.end(); ) {
          auto current = it++;
          if (current->second==0) numRREQ_forwarded.erase(current->first);
          else EV<<"host IP: "<<current->first<<" num RREQ fwd " << current->second<<endl;
    }
    ...
    if (rrep->getDestAddr()!=sourceAddr){  // Khong phai la node dich gui
        ...
        EV_INFO << "Node vừa gửi RREP cho tôi là: "<< sourceAddr<<endl;
        EV_INFO << "Lượng RREQ "<<sourceAddr<<" đã chuyển tiếp (hoặc tạo ra và gửi): "<<numRREQ_forwarded[sourceAddr]<<endl;

        if ((numRREQ_forwarded[sourceAddr]==0) && (rrep->getHopCount()==1)) {
            EV_INFO << "RREP của node độc hại, xóa bỏ, IP node độc hại: " << sourceAddr<<endl;
            //delete rrep;
            PrintOutmyRoutingTable();
            return;
        }
    ...
    if (getSelfIPAddress() != rrep->getOriginatorAddr()) {
        ...
        if (originatorRoute && originatorRoute->getSource() == this) {
            ...
            if (simTime() > rebootTime + deletePeriod || rebootTime == 0) {
                ...
                EV_INFO << "Chuyen RREP (ms): " <<simTime().dbl() * 1000<<endl;
...
void TnTaodv::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void TnTaodv::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void TnTaodv::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void TnTaodv::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void TnTaodv::socketClosed(UdpSocket *socket)
{
...
void TnTaodv::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{

    //thomc hack
    EV_INFO<<"I have just receive a RREQ from "<< sourceAddr << endl;
    numRREQ_forwarded[sourceAddr]++;
    for (auto it = numRREQ_forwarded.begin(); it != numRREQ_forwarded.end(); ) {
             auto current = it++;
             if (current->second==0) numRREQ_forwarded.erase(current->first);
             else EV<<"host IP: "<<current->first<<" num RREQ fwd " << current->second<<endl;
...
//        mynumRREQ++;
//        EV_INFO<<"May Current RREQ count is " << mynumRREQ<< endl;
    }
    else
        EV_WARN << "Can't forward the RREQ because of its small (<= 1) TTL: " << timeToLive << " or the AODV reboot has not completed yet" << endl;

    //hack
    EV_INFO<<"Ham hanldeRREQ-";
    PrintOutmyRoutingTable();
}

IRoute *TnTaodv::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void TnTaodv::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void TnTaodv::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> TnTaodv::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void TnTaodv::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void TnTaodv::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void TnTaodv::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void TnTaodv::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void TnTaodv::clearState()
{
...
void TnTaodv::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void TnTaodv::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void TnTaodv::forwardRREQ(const Ptr<Rreq>& rreq, unsigned int timeToLive)
{
    ...
    // Ghi nhan thoi gian quang bá
   // lastBroadcastTime =simtim();
}

void TnTaodv::completeRouteDiscovery(const L3Address& target)
{
...
void TnTaodv::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rrep> TnTaodv::createHelloMessage()
{
...
void TnTaodv::sendHelloMessagesIfNeeded()
{
...
void TnTaodv::handleHelloMessage(const Ptr<Rrep>& helloMessage)
{
...
void TnTaodv::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result TnTaodv::datagramForwardHook(Packet *datagram)
{
...
void TnTaodv::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void TnTaodv::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool TnTaodv::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> TnTaodv::createRREPACK()
{
...
void TnTaodv::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void TnTaodv::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void TnTaodv::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void TnTaodv::handleBlackListTimer()
{
