...
// black hole attack
    if (not strcmp(msg->getFullName(), "blackholeActivate")) {
       ...
       //     delete(msg);
            ...
            NTU_BlackholeMessage *bhmsg;
            bhmsg = check_and_cast<NTU_BlackholeMessage *>(msg);
            LOG << "Deactivating module TnTaodv_BH for Blackhole...\n";
      ...
      //      delete(msg);
        ...
        LOG << "--> Activating module TnTaodv_BH for Sinkhole...\n";
  ...
  //      delete(msg);
        ...
        NTU_SinkholeMessage *shmsg;
        shmsg = check_and_cast<NTU_SinkholeMessage *>(msg);
        LOG << "Deactivating module TnTaodv_BH for Sinkhole...\n";
        //Now sinkholes attack is deactivated
  ...
  //      delete(msg);
    ...
    /*-------------------------- GRAYHOLE ATTACK -------------------------*/
        ...
        LOG << "--> Activating module TnTaodv_BH for Grayhole...\n";
    ...
    //    delete(msg);
        ...
        NTU_GrayholeMessage *ghmsg;
        ghmsg = check_and_cast<NTU_GrayholeMessage *>(msg);
        LOG << "Deactivating module TnTaodv_BH for Grayhole...\n";
        //Now sinkholes attack is deactivated
    ...
    //    delete(msg);
     // END NTU_GRAYKHOLE -
        ...
        LOG << "ERROR: Message unknown in TnTaodv_BH::handleMessageFromAttackController. Msg: " << msg->getFullName() << "\n";
    }
}

//----------------end attack code----------------------
void TnTaodv_BH::initialize(int stage)
{
...
void TnTaodv_BH::handleMessageWhenUp(cMessage *msg)
{
...
void TnTaodv_BH::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
 ...
 *
 * */
void TnTaodv_BH::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result TnTaodv_BH::ensureRouteForDatagram(Packet *datagram)
{
...
TnTaodv_BH::TnTaodv_BH()
{

}

bool TnTaodv_BH::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void TnTaodv_BH::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address TnTaodv_BH::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void TnTaodv_BH::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void TnTaodv_BH::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void TnTaodv_BH::sendRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rreq> TnTaodv_BH::createRREQ(const L3Address& destAddr)
{
...
const Ptr<Rrep> TnTaodv_BH::createRREP(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
...
const Ptr<Rrep> TnTaodv_BH::createGratuitousRREP(const Ptr<Rreq>& rreq, IRoute *originatorRoute)
{
...
void TnTaodv_BH::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
    ...
    //thomc hack
      // kiem tra do tin cay cua node vua chuyen rrep (soursce)
      if (rrep->getDestAddr()!=sourceAddr){  // Khong phai la node dich gui
          // Kiểm tr số lượng, nếu nó (nút gửi) chưa forward bất kỳ RREQ nào, hoặc tạo ra RREQ
          // vậy nó là black hole, delete gói này
  //        EV_INFO << "Khong phai node dich gui RREP"<<endl;
  //        EV_INFO << "Node yều cầu tuyến"<< rrep->getOriginatorAddr()<<endl;
  //        EV_INFO << "Node đích cần tới là"<< rrep->getDestAddr()<< endl;
  //        EV_INFO << "Trong khi node vừa gửi RREP cho tôi là"<< sourceAddr<<endl;
  //        EV_INFO << "Lượng RREQ đã chuyển tiếp (hoặc tạo ra và gửi)"<<numRREQ_forwarded[sourceAddr]<<endl;
          if (numRREQ_forwarded[sourceAddr]==0) {
              EV_INFO << "RREP của node độc hại, xóa bỏ";
              //delete rrep;
              return;
          }
      }
      //else EV_INFO << "Đúng là node đích được yêu cầu vừa gửi cho tôi, tôi sẽ chuyển tiếp"<<endl;
...
void TnTaodv_BH::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void TnTaodv_BH::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void TnTaodv_BH::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void TnTaodv_BH::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void TnTaodv_BH::socketClosed(UdpSocket *socket)
{
...
void TnTaodv_BH::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
    numRREQ_forwarded[sourceAddr]++;  //thomc hack
...
IRoute *TnTaodv_BH::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void TnTaodv_BH::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void TnTaodv_BH::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> TnTaodv_BH::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void TnTaodv_BH::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void TnTaodv_BH::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void TnTaodv_BH::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void TnTaodv_BH::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void TnTaodv_BH::clearState()
{
    rerrCount = rreqCount = rreqId = sequenceNum = 0;
    addressToRreqRetries.clear();
    numRREQ_forwarded.clear();
    numRREQ_received.clear();
...
void TnTaodv_BH::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void TnTaodv_BH::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void TnTaodv_BH::forwardRREQ(const Ptr<Rreq>& rreq, unsigned int timeToLive)
{
    EV_INFO << "Yes. Forwarding the Route Request message with TTL= " << timeToLive << endl;
    sendAODVPacket(rreq, addressType->getBroadcastAddress(), timeToLive, *jitterPar);

}

void TnTaodv_BH::completeRouteDiscovery(const L3Address& target)
{
...
void TnTaodv_BH::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rrep> TnTaodv_BH::createHelloMessage()
{
...
void TnTaodv_BH::sendHelloMessagesIfNeeded()
{
...
void TnTaodv_BH::handleHelloMessage(const Ptr<Rrep>& helloMessage)
{
...
void TnTaodv_BH::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result TnTaodv_BH::datagramForwardHook(Packet *datagram)
{
...
void TnTaodv_BH::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void TnTaodv_BH::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool TnTaodv_BH::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> TnTaodv_BH::createRREPACK()
{
...
void TnTaodv_BH::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void TnTaodv_BH::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void TnTaodv_BH::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void TnTaodv_BH::handleBlackListTimer()
{
