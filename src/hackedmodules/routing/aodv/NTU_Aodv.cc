...
Define_Module(NTU_Aodv);

const int KIND_DELAYEDSEND = 100;

/* NTU_ATTACKS: handle message from controller. */
void NTU_Aodv::handleMessageFromAttackController(cMessage *msg){
    ...
    Enter_Method("NTU_Aodv: handle message from attack controller");
    LOG << "NTU_Aodv: Received message: "<< msg->getFullName() << "\n";
    // black hole attack
    if (not strcmp(msg->getFullName(), "blackholeActivate")) {
            NTU_BlackholeMessage *bhmsg;
            bhmsg = check_and_cast<NTU_BlackholeMessage *>(msg);
            LOG << "--> Activating module NTU_Aodv for Blackhole...\n";
            LOG << "    Blackhole Probability received: "<< bhmsg->getBlackholeAttackProbability() << "\n";
            LOG << "    Black only when route in table: " << bhmsg->getBlackOnlyWhenRouteInTable() << "\n";
            //Now sinkhole attack is activated in this module
            blackholeAttackIsActive = true;
            blackholeAttackProbability = bhmsg->getBlackholeAttackProbability();
            blackOnlyWhenRouteInTable = bhmsg->getBlackOnlyWhenRouteInTable();
            seqnoAdded = bhmsg->getSeqnoAdded();
            numHops = bhmsg->getNumHops();
        } else if (not strcmp(msg->getFullName(), "blackholeDeactivate")) {
            NTU_BlackholeMessage *bhmsg;
            bhmsg = check_and_cast<NTU_BlackholeMessage *>(msg);
            LOG << "Deactivating module NTU_Aodv for Blackhole...\n";
      ...
      //      delete(msg);
        ...
        }else if (not strcmp(msg->getFullName(), "sinkholeActivate")) {
        NTU_SinkholeMessage *shmsg;
        shmsg = check_and_cast<NTU_SinkholeMessage *>(msg);
        LOG << "--> Activating module NTU_Aodv for Sinkhole...\n";
        LOG << "    Sinkhole Probability received: "<< shmsg->getSinkholeAttackProbability() << "\n";
        LOG << "    Sink only when route in table: " << shmsg->getSinkOnlyWhenRouteInTable() << "\n";
        //Now sinkhole attack is activated in this module
        sinkholeAttackIsActive = true;
        sinkholeAttackProbability = shmsg->getSinkholeAttackProbability();
        sinkOnlyWhenRouteInTable = shmsg->getSinkOnlyWhenRouteInTable();
        seqnoAdded = shmsg->getSeqnoAdded();
        numHops = shmsg->getNumHops();
  //      delete(msg);
        // Deactivate sinkhole
    } else if (not strcmp(msg->getFullName(), "sinkholeDeactivate")) {
        NTU_SinkholeMessage *shmsg;
        shmsg = check_and_cast<NTU_SinkholeMessage *>(msg);
        LOG << "Deactivating module NTU_Aodv for Sinkhole...\n";
        //Now sinkholes attack is deactivated
  ...
  //      delete(msg);
     // END NTU_SINKHOLE -
    /*-------------------------- GRAYHOLE ATTACK -------------------------*/
    } else if (not strcmp(msg->getFullName(), "grayholeActivate")) {
        NTU_GrayholeMessage *ghmsg;
        ghmsg = check_and_cast<NTU_GrayholeMessage *>(msg);
        LOG << "--> Activating module NTU_Aodv for Grayhole...\n";
        LOG << "    Grayhole Probability received: "<< ghmsg->getGrayholeAttackProbability() << "\n";
        LOG << "    Gray only when route in table: " << ghmsg->getGrayOnlyWhenRouteInTable() << "\n";
        //Now sinkhole attack is activated in this module
        grayholeAttackIsActive = true;
        grayholeAttackProbability = ghmsg->getGrayholeAttackProbability();
        grayOnlyWhenRouteInTable = ghmsg->getGrayOnlyWhenRouteInTable();
        seqnoAdded = ghmsg->getSeqnoAdded();
        numHops = ghmsg->getNumHops();
    //    delete(msg);
        // Deactivate sinkhole
    } else if (not strcmp(msg->getFullName(), "grayholeDeactivate")) {
        NTU_GrayholeMessage *ghmsg;
        ghmsg = check_and_cast<NTU_GrayholeMessage *>(msg);
        LOG << "Deactivating module NTU_Aodv for Grayhole...\n";
        //Now sinkholes attack is deactivated
        grayholeAttackIsActive = false;
        seqnoAdded = 0;
        numHops = 0;
    //    delete(msg);
     // END NTU_GRAYKHOLE - begin wormhole  wormhole
    } else if (not strcmp(msg->getFullName(), "losauActivate")) {
        NTU_WormholeMessage *whmsg;

        whmsg = check_and_cast<NTU_WormholeMessage *>(msg);
        LOG << "--> Activating module NTU_Aodv for Wormhole...\n";
        LOG << "    Wormhole Probability received: "<< whmsg->getWormholeAttackProbability() << "\n";
        wormholeAttackIsActive = true;
        wormholeAttackProbability = whmsg->getWormholeAttackProbability();

    } else if (not strcmp(msg->getFullName(), "losauDeactivate")) {
        NTU_WormholeMessage *whmsg;
        whmsg = check_and_cast<NTU_WormholeMessage *>(msg);
        LOG << "Deactivating module NTU_Aodv for Wormhole...\n";
        wormholeAttackIsActive = false;
     // END NTU_wormKHOLE -
    }    else {

        LOG << "ERROR: Message unknown in NTU_Aodv::handleMessageFromAttackController. Msg: " << msg->getFullName() << "\n";
    }
}

//----------------end attack code----------------------
void NTU_Aodv::initialize(int stage)
{

    // Grayhole attack initializtion
    grayholeAttackIsActive = false;
    grayholeAttackProbability=1;
    grayOnlyWhenRouteInTable=false;
    grayholeDroppingProbability=0;

    // Sinkhole attack initialization
    sinkholeAttackIsActive = false;
    sinkholeAttackProbability = 1;
...
void NTU_Aodv::handleMessageWhenUp(cMessage *msg)
{
...
void NTU_Aodv::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
 ...
 *
 * */
void NTU_Aodv::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result NTU_Aodv::ensureRouteForDatagram(Packet *datagram)
{
...
NTU_Aodv::NTU_Aodv()
{
    // thomc: step2
       ...
       TrustScoreNira=0; // all nodes is untrusted
}

bool NTU_Aodv::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void NTU_Aodv::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address NTU_Aodv::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void NTU_Aodv::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void NTU_Aodv::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void NTU_Aodv::sendRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rreq> NTU_Aodv::createRREQ(const L3Address& destAddr)
{
...
const Ptr<Rrep> NTU_Aodv::createRREP(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
    ...
    // sinkhole/grayhole/blackhole attack
    if ( ((sinkholeAttackIsActive ==true) && (0< sinkholeAttackProbability))||
      //   ((grayholeAttackIsActive==true) && (0< grayholeAttackProbability)) ||
         ((blackholeAttackIsActive==true) && (0< blackholeAttackProbability))       )
    {
        rrep->setDestSeqNum(seqnoAdded);
        rrep->setHopCount(numHops);
...
const Ptr<Rrep> NTU_Aodv::createGratuitousRREP(const Ptr<Rreq>& rreq, IRoute *originatorRoute)
{
...
void NTU_Aodv::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
    ...
    if (getSelfIPAddress() != rrep->getOriginatorAddr()) {
        ...
        if (originatorRoute && originatorRoute->getSource() == this) {
            ...
            if (simTime() > rebootTime + deletePeriod || rebootTime == 0) {
                ...
                //hack
                if (( grayholeAttackIsActive==true) && (0 <grayholeAttackProbability))
                   {
                        rrep->setHopCount(1);
                ...
                //thomc: Step 6, and 7
                /* If it is destination then it will send reply using sendReply() function.
                 * Trust value is increment by 1 and assigned to all nodes in the path
                 * from destination to source node.
                 * Now, Source becomes destination for the current node.
                 * */
                TrustScoreNira += rrep->getTrustScoreNira();
            }
        }
        else
...
void NTU_Aodv::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void NTU_Aodv::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
    ...
    if (wormholeAttackIsActive)
        {   // Get IP infos
            L3Address Wormholer_A_eth0 = inet::L3Address("145.237.1.100");
            L3Address Wormholer_B_eth0 = inet::L3Address("145.237.1.200");
            L3Address Wormholer_A_wan0 = inet::L3Address("145.236.1.10");
            L3Address Wormholer_B_wan0 = inet::L3Address("145.236.1.20");
            L3Address myCurrentIPL3= getSelfIPAddress();
            // Add fixed route A-->B
            createRouteWired(Wormholer_B_eth0, Wormholer_B_eth0, 1, false, 100, true, simTime() + activeRouteTimeout);
            PrintOutmyRoutingTable();
            if (myCurrentIPL3 == Wormholer_A_eth0 ) {
               if( aodvPacket->getPacketType() == RREQ ) {  // By default: A send RREQ to B over wired link
                       int interfaceId = CHK(interfaceTable->getInterface(2))->getInterfaceId(); //2 = ethernet interface
                       packet->addTag<InterfaceReq>()->setInterfaceId(interfaceId);
                       packet->addTag<HopLimitReq>()->setHopLimit(timeToLive);
                       packet->addTag<L3AddressReq>()->setDestAddress(Wormholer_B_eth0);
                       packet->addTag<L4PortReq>()->setDestPort(aodvUDPPort);
                   }
               if( aodvPacket->getPacketType() == RREP ){  // By default: A broadcast RRER to others over wireless link
                      int interfaceId = CHK(interfaceTable->getInterface(1))->getInterfaceId(); //1 = wireless interface
                      packet->addTag<InterfaceReq>()->setInterfaceId(interfaceId);
                      packet->addTag<HopLimitReq>()->setHopLimit(timeToLive);
            ...
            if (myCurrentIPL3 == Wormholer_B_eth0 ) {
                 if( aodvPacket->getPacketType() == RREQ ) {
                     int interfaceId = CHK(interfaceTable->getInterface(1))->getInterfaceId(); //1 = wireless interface
                     packet->addTag<InterfaceReq>()->setInterfaceId(interfaceId);
                     packet->addTag<HopLimitReq>()->setHopLimit(timeToLive);
                     packet->addTag<L3AddressReq>()->setDestAddress(destAddr);
                 ...
                 if( aodvPacket->getPacketType() == RREP ){
                     int interfaceId = CHK(interfaceTable->getInterface(2))->getInterfaceId(); //2 = ethernet interface
                    packet->addTag<InterfaceReq>()->setInterfaceId(interfaceId);
                    packet->addTag<HopLimitReq>()->setHopLimit(timeToLive);
                    packet->addTag<L3AddressReq>()->setDestAddress(Wormholer_A_eth0);
                    packet->addTag<L4PortReq>()->setDestPort(aodvUDPPort);
                 }
          }
    ...
    if (delay == 0) {
        ...
        //thomc hack
                    // Nếu là tấn công, thì không resend lại sau khoảng Jitter
                       /*--------------------Attack code begin------------------------------------------------ */
                       /*------Xử lý phát lại sau khoảng Jitte--------------------------------------------------------------------------------*/
                       // begin attack
                if (aodvPacket->getPacketType() == RREQ)
                if (  ((sinkholeAttackIsActive ==true) && (0<sinkholeAttackProbability))||
                    //   ((grayholeAttackIsActive==true) && (0 <grayholeAttackProbability)) ||
                       ((blackholeAttackIsActive==true) && (0<blackholeAttackProbability))       )
                {
                       EV_INFO<<"No, I'm a hacker, i do not forwarding RREQ after aodv-send-jitter"<< endl;
...
void NTU_Aodv::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void NTU_Aodv::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void NTU_Aodv::socketClosed(UdpSocket *socket)
{
...
void NTU_Aodv::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
    ...
    // check (i)
        ...
        if (timeToLive > 0 && (simTime() > rebootTime + deletePeriod || rebootTime == 0)) {
            if (destRouteData) rreq->setDestSeqNum(std::max(destRouteData->getDestSeqNum(), rreq->getDestSeqNum()));
...
IRoute *NTU_Aodv::createRouteWired(const L3Address& destAddr, const L3Address& nextHop,
        ...
        bool isActive, simtime_t lifeTime)
{
    ...
    InterfaceEntry *ifEntry = interfaceTable->getInterface(2); // TODO: IMPLEMENT: multiple interfaces
    if (ifEntry)
...
IRoute *NTU_Aodv::createRoute(const L3Address& destAddr, const L3Address& nextHop,
        unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum,
        bool isActive, simtime_t lifeTime)
{
    // create a new route
...
void NTU_Aodv::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void NTU_Aodv::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> NTU_Aodv::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void NTU_Aodv::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void NTU_Aodv::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void NTU_Aodv::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void NTU_Aodv::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void NTU_Aodv::clearState()
{
...
void NTU_Aodv::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void NTU_Aodv::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
//           // Cu phap ham send
//             //void NTU_Aodv::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
//           //Cau hinh IP trong file ini
//    //       <interface hosts='Wormholer_A' names='eth0' address='10.0.0.2' netmask='255.0.0.0' /> \
//    //     <interface hosts='Wormholer_B' names='eth0' address='10.0.0.1' netmask='255.0.0.0' /> \
//    //     <interface hosts='Wormholer_A' names='wlan0' address='145.236.0.2' netmask='255.255.0.0' /> \
//    //     <interface hosts='Wormholer_B' names='wlan0' address='145.236.0.1' netmask='255.255.0.0' />
//           //       L3Address myIPL3= getSelfIPAddress();
//
//
//           L3Address Wormholer_A_eth0 = inet::L3Address("10.0.0.2");
//           L3Address Wormholer_B_eth0 = inet::L3Address("10.0.0.1");
//
//           L3Address Wormholer_A_wan0 = inet::L3Address("145.236.0.2");
//           L3Address Wormholer_B_wan0 = inet::L3Address("145.236.0.1");
//
//           // My wan CURENT IP
//           L3Address myIPL3= getSelfIPAddress();
//           if (myIPL3 == Wormholer_A_wan0 )
//           {
//               sendAODVPacket(rreq,Wormholer_B_eth0 , timeToLive, *jitterPar);
//           }
//           if (myIPL3 == Wormholer_B_wan0 )
//                  {
//                   sendAODVPacket(rreq,Wormholer_A_eth0 , timeToLive, *jitterPar);
//           }
//       }
//    else
...
void NTU_Aodv::forwardRREQ(const Ptr<Rreq>& rreq, unsigned int timeToLive)
{
//   EV_INFO << "Forwarding the Route Request message with TTL= " << timeToLive << endl;
//   L3Address Wormholer_A_eth0 = inet::L3Address("10.0.0.1");
//   L3Address Wormholer_B_eth0 = inet::L3Address("10.0.0.2");
//   L3Address Wormholer_A_wan0 = inet::L3Address("145.236.0.1");
//   L3Address Wormholer_B_wan0 = inet::L3Address("145.236.0.2");
//   L3Address myIPL3= getSelfIPAddress();
//   if ((wormholeAttackIsActive) && (myIPL3 == Wormholer_A_wan0 ) )
//       sendAODVPacket(rreq,Wormholer_A_wan0 , timeToLive, *jitterPar);
//   else
      sendAODVPacket(rreq, addressType->getBroadcastAddress(), timeToLive, *jitterPar);
}

void NTU_Aodv::completeRouteDiscovery(const L3Address& target)
{
...
void NTU_Aodv::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rrep> NTU_Aodv::createHelloMessage()
{
...
void NTU_Aodv::sendHelloMessagesIfNeeded()
{
...
void NTU_Aodv::handleHelloMessage(const Ptr<Rrep>& helloMessage)
{
...
void NTU_Aodv::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result NTU_Aodv::datagramForwardHook(Packet *datagram)
{
...
void NTU_Aodv::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void NTU_Aodv::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool NTU_Aodv::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> NTU_Aodv::createRREPACK()
{
...
void NTU_Aodv::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void NTU_Aodv::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void NTU_Aodv::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void NTU_Aodv::handleBlackListTimer()
{
...
void NTU_Aodv::PrintOutmyRoutingTable(){
    EV_INFO <<"--- --------------ROUTING TABBLE at " <<simTime() <<"XEM"<<endl;
       EV_INFO<<"Dest ------------ NextHop ---- HC -----SSN ---DSN"<<endl;
       for (int i = 0; i < routingTable->getNumRoutes(); i++) {
               IRoute *route = routingTable->getRoute(i);
               if (route->getSource() == this) { // Chinhs la node hien hanh
                    AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
                    EV_INFO<<"| -- "<< route->getDestinationAsGeneric ();
                    EV_INFO<<"| -- "<<  route->getNextHopAsGeneric();
                    EV_INFO <<"| -- "<< route->getMetric();
                    EV_INFO <<"| -- "<<  sequenceNum;
                    EV_INFO <<"| -- "<<  routeData ->getDestSeqNum() <<endl;  //nhaanj duco
