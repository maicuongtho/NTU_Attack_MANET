...
Define_Module(floodNTU_Aodv);

const int KIND_DELAYEDSEND = 100;

/* floodNTU_ATTACKS: handle message from controller. */
void floodNTU_Aodv::handleMessageFromAttackController(cMessage *msg){
    Enter_Method("floodNTU_Aodv: handle message from attack controller");
    LOG << "floodNTU_Aodv: Received message: "<< msg->getFullName() << "\n";

    //  flooding attack
    if (not strcmp(msg->getFullName(), "floodingActivate")) {
            NTU_FloodingM *fmsg;
            fmsg = check_and_cast<NTU_FloodingM*>(msg);
            LOG << "--> Activating module floodNTU_Aodv for Flooding...\n";
        ...
        LOG << "ERROR: Message unknown in floodNTU_Aodv::handleMessageFromAttackController. Msg: " << msg->getFullName() << "\n";
    }
}

//----------------end attack code----------------------
void floodNTU_Aodv::initialize(int stage)
{
    // end attack init
...
void floodNTU_Aodv::handleMessageWhenUp(cMessage *msg)
{
...
void floodNTU_Aodv::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
 ...
 *
 * */
void floodNTU_Aodv::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result floodNTU_Aodv::ensureRouteForDatagram(Packet *datagram)
{
...
floodNTU_Aodv::floodNTU_Aodv()
{
}

bool floodNTU_Aodv::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void floodNTU_Aodv::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address floodNTU_Aodv::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void floodNTU_Aodv::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void floodNTU_Aodv::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void floodNTU_Aodv::sendFakeRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
    // In an expanding ring search, the originating node initially uses a TTL =
    // TTL_START in the RREQ packet IP header and sets the timeout for
    // receiving a RREP to RING_TRAVERSAL_TIME milliseconds.
...
void floodNTU_Aodv::sendRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rreq> floodNTU_Aodv::createRREQ(const L3Address& destAddr)
{
...
const Ptr<Rrep> floodNTU_Aodv::createRREP(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
...
const Ptr<Rrep> floodNTU_Aodv::createGratuitousRREP(const Ptr<Rreq>& rreq, IRoute *originatorRoute)
{
...
void floodNTU_Aodv::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
...
void floodNTU_Aodv::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void floodNTU_Aodv::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void floodNTU_Aodv::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void floodNTU_Aodv::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void floodNTU_Aodv::socketClosed(UdpSocket *socket)
{
...
void floodNTU_Aodv::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
...
IRoute *floodNTU_Aodv::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void floodNTU_Aodv::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void floodNTU_Aodv::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> floodNTU_Aodv::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void floodNTU_Aodv::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void floodNTU_Aodv::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void floodNTU_Aodv::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void floodNTU_Aodv::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void floodNTU_Aodv::clearState()
{
    ...
    if (rrepAckTimer)
        cancelEvent(rrepAckTimer);
    if (rreqFloodingTimer)
        cancelEvent(rreqFloodingTimer);
}

void floodNTU_Aodv::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void floodNTU_Aodv::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void floodNTU_Aodv::forwardRREQ(const Ptr<Rreq>& rreq, unsigned int timeToLive)
{
...
void floodNTU_Aodv::completeRouteDiscovery(const L3Address& target)
{
...
void floodNTU_Aodv::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rrep> floodNTU_Aodv::createHelloMessage()
{
...
void floodNTU_Aodv::sendHelloMessagesIfNeeded()
{
...
void floodNTU_Aodv::handleHelloMessage(const Ptr<Rrep>& helloMessage)
{
...
void floodNTU_Aodv::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        IRoute *route = routingTable->getRoute(i);
        if (route->getSource() == this) {
...
void floodNTU_Aodv::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result floodNTU_Aodv::datagramForwardHook(Packet *datagram)
{
...
void floodNTU_Aodv::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void floodNTU_Aodv::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool floodNTU_Aodv::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> floodNTU_Aodv::createRREPACK()
{
...
void floodNTU_Aodv::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void floodNTU_Aodv::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void floodNTU_Aodv::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void floodNTU_Aodv::handleBlackListTimer()
{
...
void floodNTU_Aodv::handleRreqFloodingTimer()
{
    ...
    L3Address destAddr;
    destAddr.set(addr);
   // auto fakeRREQ= createFakeRREQ(destAddr);
